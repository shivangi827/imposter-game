...
// ─── HTTP Security ────────────────────────────────────────────────────────────
app.set('trust proxy', 1); // we run behind Fly's proxy + Cloudflare
app.use(
  helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc: ["'self'"],
        scriptSrc: ["'self'", 'https://www.clarity.ms', 'https://c.clarity.ms'],
        styleSrc: ["'self'", "'unsafe-inline'"],
        imgSrc: ["'self'", 'data:', 'https://c.clarity.ms'],
        connectSrc: ["'self'", 'https://www.clarity.ms', 'https://c.clarity.ms', 'wss:', 'ws:'],
        fontSrc: ["'self'"],
        objectSrc: ["'none'"],
        frameAncestors: ["'none'"],
      },
...
app.use(
  rateLimit({
    windowMs: 60_000,
    limit: 120, // 120 HTTP requests/min per IP
...
// ─── Socket.IO with payload cap ───────────────────────────────────────────────
const io = new Server<ClientToServerEvents, ServerToClientEvents, Record<string, never>, SocketData>(
  server,
  {
    maxHttpBufferSize: 4 * 1024, // 4 KB — way more than we ever need
    pingTimeout: 20_000,
    pingInterval: 25_000,
  }
);
...
function getIp(socket: GameSocket): string {
...
io.use((socket, next) => {
  const ip = getIp(socket as GameSocket);
  const set = socketsByIp.get(ip) ?? new Set<string>();
  if (set.size >= MAX_SOCKETS_PER_IP) {
...
io.on('connection', (socket: GameSocket) => {
  ...
  socket.on('start-game', () => {
    ...
    for (const player of room.players) {
      const sock = io.sockets.sockets.get(player.id) as GameSocket | undefined;
      if (!sock) continue;
  ...
  socket.on('submit-vote', ({ votedId }) => {
    ...
    io.to(room.code).emit('vote-update', { votedCount, totalPlayers });
...
server.listen(PORT, () => {
  console.log(`Imposter game running → http://localhost:${PORT}`);
});
